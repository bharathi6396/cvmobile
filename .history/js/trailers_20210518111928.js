...
$(function () {
    ...
    $.ajax({
        ...
        success: function (data, status, response) {
            ...
            let filmstrailers = data.results;
            filmstrailers.forEach(element => {
                let addBack = `<a href="#" class="movie-item"><iframe src="${URL_YOUTUBE + element.key}"frameborder="1" allowfullscreen></iframe><div><div> "${element.name}"  </div> <p1> "${element.site}"</p> <p> "${element.type}"</p> </div ></a>`;
                $(".scrollside-trailers").append(addBack);
            });
        },
    });
