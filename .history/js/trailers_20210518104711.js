    ...
    $.ajax({
        ...
        success: function (data, statuts, response) {
            ...
            trailer.forEach(function (element) {
                console.log(element)
                let lien = `<iframe src="${YOUTUBE_URL + element.key}"frameborder="1" allowfullscreen></iframe>`;
                $(".scrollmenu-trailer").append(lien);
            });
        }
    });
});
    ...
    $.ajax({
        ...
        success: function (data, status, response) {
            ...
            filmstrailers.forEach(element => {
                let addBack = `<a href="film.html?filmId=${element.id}" class="movie-item"> <video src="${URL_YOUTUBE + element.id}"><div id="flexbox"><div class="change"> ${element.name}  </div> <p> ${element.release_date}</p> </div> </a>`;
