    ...
    $.ajax({
        ...
        success: function (data, status, response) {
            ...
            filmsPopular.forEach(element => {
                let addBack = `<a href="film.html?filmId=${element.id}" class="movie-item"> <img src="${URL_IMAGE_PREFIX + element.poster_path}"><div id="flexbox"> <div class="vote ${element.vote_average < 7 : 'red' : ''}">${element.vote_average}</div> <div class="change"> ${element.original_title}   </div> <p> ${element.release_date}</p> </div> </a>`;
