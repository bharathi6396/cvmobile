// $('.glyphicon-plus img').tooltip({ boundary: 'window' })
        ...
        $.ajax({
            ...
            success: function (data, statuts, response) {
                ...
                moviesOntv.forEach(function (element) {
                    let ToAdd = `<a href="#" class="movie-item"><img src="${URL_IMAGE_PREFIX + element.poster_path}"><div class="vote${element.vote_average < 7 ? 'low' : ''}">${element.vote_average}</div> <div><div> "${element.original_title}"  </div> <p> "${element.release_date}"</> </div ></a>`;
