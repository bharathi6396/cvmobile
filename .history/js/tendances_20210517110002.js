...
$.ajax({
        ...
        success: function (data, statuts, response) {
            ...
            moviesTendance.forEach(function (element) {
                let moviesTendanceToAdd = `<a href="film.html?filmId=${element.id}" class="movie-item"> <class="img" img src="${URL_IMAGE_PREFIX + element.poster_path}"> <div><div class="change"> ${element.original_title}  </div> <p> ${element.release_date}</p> </div> </a>`;
                $(".scrollmenu-tendance-day").append(moviesTendanceToAdd);
            });
        },   
    });
    $(".week").click(function () {
       // $(".scrollmenu-tendance-day").hide();
